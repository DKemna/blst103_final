"""
HOLC Redlining and Education Correlation Analysis – Los Angeles, California
===========================================================================
This script analyzes the correlation between 1930s HOLC redlining grades
in Los Angeles and current educational outcomes by census tract.

This script uses Census Bureau ACS data on educational attainment for the
population 25+. The key metrics are:
...
def download_holc_data():
    """Download and cache Los Angeles HOLC redlining GeoJSON."""
    cache_path = os.path.join(script_dir, "geojson.json")
    if not os.path.exists(cache_path):
        print("  Downloading Los Angeles HOLC data from Mapping Inequality...")
        urls = [
            "https://dsl.richmond.edu/panorama/redlining/static/citiesData/CALosAngeles1939/geojson.json",
            "https://dsl.richmond.edu/panorama/redlining/static/downloads/geojson/CALosAngeles1939.geojson",
        ]
        ...
        for url in urls:
            ...
            raise RuntimeError("Could not download Los Angeles HOLC data.")
    return gpd.read_file(cache_path)


def download_tract_boundaries():
    """Download census tract boundaries for California, filter to LA County."""
    cache_path = os.path.join(script_dir, "tracts_ca.zip")
    urls = [
    ...
    if not os.path.exists(cache_path):
        print("  Downloading California census tract boundaries...")
        for url in urls:
            try:
    ...
    tracts = tracts[tracts["COUNTYFP"].isin(LA_COUNTIES)].copy()
    return tracts
...
print("\nDownloading Los Angeles-area census tract boundaries...")
...
print("\n" + "=" * 70)
print("  STATISTICAL RESULTS — HOLC Grade vs Educational Attainment (Los Angeles)")
print("=" * 70)
...
ax1.set_title(
    f"HOLC Score vs College Attainment in Los Angeles\nr={r:.3f}, p={p:.4f}",
    fontsize=13,
    fontweight="bold",
)
...
ax2.set_title(
    "College Attainment by Dominant HOLC Grade – Los Angeles",
    fontsize=13,
    fontweight="bold",
)
...
ax3.set_title(
    f"HOLC Score vs % No High School Diploma in Los Angeles\nr={r3:.3f}, p={p3:.6f}",
    fontsize=13,
    fontweight="bold",
)
...
ax4.set_axis_off()
ax4.set_title(
    "Educational Attainment Map with HOLC Zone Boundaries – Los Angeles",
    fontsize=13,
    fontweight="bold",
)
